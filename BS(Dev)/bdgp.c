...
const float		fNormalBDSPFactor = 0.285714f;
...
{
	BDGP_WriteParametr(1 + rangeIndex,0x93,0,(uint8*)&BDGPParametrs[rangeIndex].fCorrectionFactors[0],sizeof(float));
}
...
void	BDGP_InsertParametrs(uint16	regID,uint8	*pData)
{
			...
			case 0x97:
			{
				...
				memcpy((uint8*)&BDGPParametrs[rangeIndex].rangeMax,pData + 6,2);
					...
					if(rangeIndex == 0)
						...
						IMDB_BDGPGetParametrsSignalCallback();
			}break;
		}
	}
	//задание параметра
	if(pData[0] == 0x04)
	{
		switch(pData[1])
		{
			...
			/*
				Получили последний параметр для I и II диапазонов
			*/
			case 0x97:
			{
				//если диапазон I - то задаём параметры для II
				if(rangeIndex == 0){
					BDGP_StartWritingParametrs(1);
				}
				//в противном случае, шлём команду на сохранение параметров первого диапазона
				else{
			...
			//получили ответ на команду записи во FLASH БДГП-Б I диапазона
			case 0xE0:
			{
				//если I диапазон, то шлём запись во FLASH для II диапазона
				if(rangeIndex == 0)
					BDGP_WriteParametr(2,0xE0,0xFF,0,0);
				else
					IMDB_BDGPWriteParametrsSignalCallback();	//callback успешной записи
			}break;
		}
	}
...
float	*BDGP_DirectionDiagramm(float	*pData, uint8	SensCount, float cntMin,float cntMax, uint16	*pAngleMax, eAngDiagrammMode Mode)
{
	...
	float fLHMin = 100000;	//минимум функции правдоподобия
	float fLHMax = -100000;	//максимум функции правдоподобия
	...
	float fFirstRecalKoef = 0;
	float fSecondRecalKoef = 0;
	
	tBDSPParametrs	*curBDSPSettings;
	
	
	if(Mode == CNTR_MODE){
		fK = 0.8f - ((cntMin + 1) / (cntMax + 1));	//коэф. разрыхления жопы
		directionCoef = BDGPdirectionCoef;
	}
	else{
		fK = sqrt((cntMin + 1) / (cntMax + 1));
		curBDSPSettings = (tBDSPParametrs*)BDSP_GetParametrs();
		directionCoef = BDSPdirectionCoef;
	}
	
	memset(fAngleDiagramm,0,sizeof(float) * 48);
	
	SignalLevel = (cntMax - cntMin) / (1 - directionCoef[180]);
	BackLevel = (cntMin - directionCoef[180] * cntMax) / (1 - directionCoef[180]);
	...
	//if(fLikelihoods)
	{
		...
		for(f = 0;f<360;f++)
		{
			for(i = 0;i<SensCount;i++)
			{
				if(Mode == CNTR_MODE){				
					...
					else if((360 - f + 90 * i) < 181 )
					{
						lIndex[i] = (360 - f + 90 * i);
					}
					else
					{
						lIndex[i] = abs(360 + f - 90 * i);
					}
				}
			}
			...
			for(i = 0;i<SensCount;i++)
			{
				...
				if(Mode == SPEC_MODE)
					fLikelihoods[f] += (pData[i] * curBDSPSettings->fAligmentFactors[i] * log(1 + fLog * directionCoef[lIndex[i]]) - SignalLevel * directionCoef[lIndex[i]]);
			}
		}
		//находим минимум 
		for(f = 0;f<360;f++)
		...
		for(f = 0;f<360;f++)
			...
			//перекалибровка функции правдоподобия для гранаты
		if(Mode == CNTR_MODE)
		{
			//вычисляем Z
		...
		//перекалибровка для спектрометра
		if(Mode == SPEC_MODE)
		{
			fFirstRecalKoef = fLHMin - ( ((fLHMax - fLHMin) * fK) / (1 - fK));
			
			//вычисляем новый максимум
			fLHMax = fLHMax - fFirstRecalKoef;//fLHMin - ( ((fLHMax - fLHMin) * fK) / (1 - fK));
			//перекалибровываем
			for(f = 0;f<360;f++)
				//fLikelihoods[f] = (fLikelihoods[f] - fLHMin - ( ((fLHMax - fLHMin) * fK) / (1 - fK) )) / fLHMax;
					fLikelihoods[f] = (fLikelihoods[f] - fFirstRecalKoef) / fLHMax;
			...
			fLHMin = pow(fLHMin, 1 + 8 * pow(fLHMin,8) );
				
			fSecondRecalKoef = (fK * fK - fLHMin) / (1 - fK * fK);
				
			//вторая перекалибровка
			for(f = 0;f<360;f++){
				//fLikelihoods[f] = (pow(fLikelihoods[f],1+8*pow(fLikelihoods[f],8)) + ( (fK*fK - fLHMin)/(1 - fK*fK) ) ) / (1 + ( (fK*fK - fLHMin)/(1 - fK*fK) ));
				fLikelihoods[f] = (pow(fLikelihoods[f], 1 + 8 * pow(fLikelihoods[f],8) ) + fSecondRecalKoef) / ( 1 + fSecondRecalKoef);
			}
...
//		if(fAngleDiagramm)
			{
				...
				if(Mode == SPEC_MODE)
				{
					if( ( (pData[0]+pData[1]+pData[2]+pData[3])) <= BDSP_GetBackgroundQueryIntegral())
						fNormizeKoef = 0.5f;
					else{
						fNormizeKoef = 1.0f - 0.5f * (BDSP_GetBackgroundQueryIntegral() /  (pData[0]+pData[1]+pData[2]+pData[3]));
					}
				}
	...
	if(BDGPData.ChangeBufferEvent == 1)
	{
		...
		//динамический движок получения средней МД
			...
			else
			{
				...
				BDGPData.Dose += (BDGPData.currentDose * DOSE_TO_HOUR);
				//BDGPData.Dose= BDGPData.Dose * DOSE_TO_HOUR;
...
uint16	BDGP_GetMaximumAngle()
{
	//float fAngle = (float)BDGPData.radiationMaximum;
	//return fAngle;
	return BDGPData.radiationMaximum;
}
float BDGP_GetDose()
{
