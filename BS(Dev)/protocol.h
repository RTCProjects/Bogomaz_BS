...
#include "main.h"
//параметр таймаута отложенного CAN сообщения
#define CALIBRATION_DEFFERED_TIME			1800000
#define SETLOGICNUMBER_DEFFERED_TIME	200000
...
#define IMDB_DEBUG_DATA_MSG					0x85	//сообщение данных из БС
...
/*структуры входящих событий*/
typedef struct
{
	uint8		requestFlag;//флаг события
	uint8		Channel;		//№ канала
	uint16	Start;			//Стартовый канал
	uint16	Count;			//Кол-во каналов
}sEventAccumSpectrBlock;

typedef struct
{
	uint8	requestFlag;
	uint8	nuclideIndex;
}sEventNuclideIdentification;
...
typedef struct
{
	...
	uint8	rSpectrAccumChanSuccess[4];
	sEventNuclideIdentification	rNuclideIdentificationCallback;
	uint8	rNuclideIdentificationEndCallback;
	uint8 rMeasureInPointEndCallback;
	//флаги исходящих событий для параметров БД
	uint8	rBDMGReadParametrsCallback;
	uint8	rBDGPReadParametrsCallback;
	uint8	rBDPSReadParametrsCallback;
	uint8	rBDMGWriteParametrsCallback;
	uint8	rBDGPWriteParametrsCallback;
	uint8	rBDPSWriteParametrsCallback;	
	...
	sEventAccumSpectrBlock	rMetrologySendAccumSpectrBlock;	//флаг отправки блока запрошенного спектра
	uint8	rMetrologyReadBDParametrs;
	uint8	rMetrologyWriteBDParametrs;
}sProtocolRequests;
...
void IMDB_ReceiveByteReading(void);
...
void IMDB_SendMeasureInPointSuccess(void);
...
void	IMDB_NuclideIdentificationEndSignalCallback(void);
void 	IMDB_SendMeasureInPointEndSignalCallback(void);

void	IMDB_BDPSGetParametrsSignalCallback(void);
void	IMDB_BDGPGetParametrsSignalCallback(void);
void	IMDB_BDMGGetParametrsSignalCallback(void);
void	IMDB_BDPSWriteParametrsSignalCallback(void);
void	IMDB_BDGPWriteParametrsSignalCallback(void);
void	IMDB_BDMGWriteParametrsSignalCallback(void);


void	IMDB_ResetAllBDSignalCallback(void);

void IMDB_Metrology_AllBDParametrRequest(void);
void IMDB_Metrology_AllBDWriteParametr(void);
