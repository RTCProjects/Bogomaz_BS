...
{/*
	...
	memset(specChannelIndex,0,sizeof(uint16) * CHANNELS);
	memset(specChannelsBuf,0,sizeof(uint16) * CHANNELS * SPECTR_SIZE);
	...
	for(i = 0;i<CHANNELS;i++)
	{
		...
		SpectrChannel[i].Index = 0;
		SpectrChannel[i].stabFlag = 0;
		SpectrChannel[i].Ready = 0;
		xmemset(SpectrChannel[i].Data,0,sizeof(uint16) * MAX_TIME * SPECTR_SIZE);

		
		
		BackgroundSpectr[i].Index = 0;
		BackgroundSpectr[i].Ready = 0;
		xmemset(BackgroundSpectr[i].Data,0,sizeof(float) * SPECTR_SIZE);
	}*/
