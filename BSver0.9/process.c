			...
			case IMDB_INITIALIZATION:
			{
				...
				if(mainCounter == 3)
				{
					...
					*/
					...
					{
						//если он есть, то необходимо замерить внешнюю МД перед включением
...
//							IMDB_SendState();
				...
				{
					/*
						TODO сделать проверку на БДМГ-Б, чтобы не происходило накопления фона при подключенном одном блоке
					*/
...
//						IMDB_SendState();
					...
					//ничего нет, шлём сообщение со статусом но не шлем событие готовности ИМДБ
					else
					{
