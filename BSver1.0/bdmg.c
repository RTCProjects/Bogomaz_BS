...
void	BDMG_InsertData(uint8	*pData)
{
	if(pData[4] == 0xFF && pData[5] == 0xFF && pData[6] == 0xFF && pData[7] == 0xFF)//приняли последнее сообщение данных с МД
	{
		...
		BDMGData.Dose += (BDMGData.currentDose * DOSE_TO_HOUR);
		//BDMGData.Dose = BDMGData.Dose * DOSE_TO_HOUR;
	}
	...
	switch(pData[0])
	{
		...
		case 0x04:
		{
			...
			if(pData[2] == 0xC2){
				...
				IMDB_BDMGGetParametrsSignalCallback();
			}
		...
		case 0x05:
		{
			switch(pData[2])
			{
				...
				case 0xC2: 
				{ 
					IMDB_BDMGWriteParametrsSignalCallback(); 
				}break;
			}
		}break;
	}
